...
| **INV-005** | 같은 주문·쿠폰 재요청은 저장된 할인 결과를 그대로 반환하며, 할인을 중복 적용하지 않는다. | 실습 조건: COND-003 | 첫 요청의 할인 결과가 저장된 뒤 같은 orderId와 couponId로 다시 요청했을 때, 두 번째 응답이 첫 번째와 다른 금액을 반환하거나 주문의 할인 효과가 한 번 더 증가함 | 저장된 할인 결과를 반환하며 할인액과 최종 금액에 추가 할인 효과가 생기지 않음 | 기존 성공 결과 조회·멱등 처리 |
